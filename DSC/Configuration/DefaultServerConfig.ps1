Configuration DefaultServerConfig
{
    ...
    Node "localhost"
    {
        File DirectoryCopy
        {
            ...
            SourcePath = "\\dccteststorage.file.core.windows.net\absg\Packages"
        ...
        {
            Ensure = "Present"
            Name = "Nimsoft Robot x64 version 7.80"
            DependsOn = "[File]DirectoryCopy"
            Path = "C:\Packages\CA\nimsoft-robot-x64.exe"
            Arguments = '/VERYSILENT /SUPPRESSMSGBOXES /NORESTART /LOG="log.txt"'
            ProductId = ""
        }
        ...
        {
            # The message below gets written to the Microsoft-Windows-Desired State Configuration/Analytic log
            Message = "Finished running the file resource with ID Install_CA"
            DependsOn = "[Package]Install_CA" # This means run "Install_CA" first.
        }
